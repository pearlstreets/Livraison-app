...
export default function DeliveryFlowScreen({ navigation, route }) {
  ...
  // Start GPS tracking when delivery begins, stop on unmount
  useEffect(() => {
    startTracking().catch(() => {});
    return () => stopTracking();
  }, []);
  ...
  const nextStep = useCallback(() => {
    if (stepIndex < STEPS.length - 1) {
      ...
      // Sync status to API
      const statusMap = { 1: 'picked_up', 2: 'en_route', 3: 'arrived', 4: 'delivered' };
      const apiStatus = statusMap[next];
      if (apiStatus && order.id) {
        deliveryService.updateDeliveryStatus(order.id, apiStatus).catch((err) => {
          // Queue for retry when back online
          offlineQueue.enqueue('updateDeliveryStatus', { assignmentId: order.id, status: apiStatus });
          setStatusError(t('statusSyncError') || 'Erreur de synchronisation. Le statut sera mis à jour automatiquement.');
        });
      }
    }
  }, [stepIndex, order.id, t]);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f5f5f5' }} edges={['top']}>
      ...
      {statusError && (
        <Pressable
          style={{ backgroundColor: '#FEF3C7', padding: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}
          onPress={() => setStatusError(null)}
          accessibilityLabel="Dismiss error"
        >
          <Ionicons name="warning" size={16} color="#92400E" style={{ marginRight: 8 }} />
          <Text style={{ color: '#92400E', fontSize: 13, flex: 1 }}>{statusError}</Text>
