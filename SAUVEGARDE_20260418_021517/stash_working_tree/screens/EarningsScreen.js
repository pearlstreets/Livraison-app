    ...
    <View style={{ flex: 1, backgroundColor: '#f5f5f5' }}>
      <ScrollView ref={scrollRef} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={s.balanceCard}>
          ...
          <Text style={s.cashoutLimit}>{t('cashoutLimit')}</Text>
        </View>
        ...
        {weeklyEarnings.length > 0 ? weeklyEarnings.map((week, i) => (
          <Pressable key={i} style={s.weekRow} onPress={() => navigation.navigate('WeekDetail', { weekIndex: i })} accessibilityLabel={`${week.range}, ${week.total.toFixed(2)} euros`}>
            <View style={{ flex: 1 }}>
          ...
          <View style={{ alignItems: 'center', paddingVertical: 40 }}>
            ...
            <Text style={{ color: '#9CA3AF', fontSize: 15, marginTop: 12 }}>{t('noEarningsYet') || 'Aucun revenu pour le moment'}</Text>
          </View>
        )}
      </ScrollView>
          ...
          <View style={s.popup}>
            {encaissStep === 'confirm' && (
              <>
                ...
                <Text style={s.popupDesc}>{cashedAmount} {t('cashoutMsg')}</Text>
            ...
            {encaissStep === 'done' && (
              <>
                ...
                <Text style={s.popupDesc}>{cashedAmount} {t('cashoutSuccessMsg')}</Text>
