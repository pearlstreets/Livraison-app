...
export default function LoginScreen() {
  ...
  // pendingValidation is now handled globally by App.js via PendingApprovalScreen
  ...
  const handleSignup = async () => {
    ...
    if (step === 2) {
      ...
      setLoading(true);
      const success = await register({ email: email.trim(), password, nom: nom.trim(), prenom: prenom.trim(), pseudo: pseudo.trim(), phone: phone.trim(), phoneCode: selectedPhoneCountry.phoneCode, country, role: 'user' });
      setLoading(false);
    ...
    setLoading(true);
    await register({ email: email.trim(), password, nom: nom.trim(), prenom: prenom.trim(), pseudo: pseudo.trim(), phone: phone.trim(), phoneCode: selectedPhoneCountry.phoneCode, country, companyName: companyName.trim(), role: 'professionaluser', isVerified: false, documents: { idFront: docIdFront, idBack: docIdBack, iban: docIban, kbiss: docKbiss } });
    setLoading(false);
    // AuthContext sets user with isVerified=false → App.js redirects to PendingApprovalScreen
  };
    ...
    <KeyboardAvoidingView style={{flex:1, backgroundColor:'#fff'}} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <SafeAreaView style={{flex:1}}>
        ...
        <ScrollView contentContainerStyle={{flexGrow:1, justifyContent:'center', paddingHorizontal:24, paddingVertical:20}} keyboardShouldPersistTaps="handled">
          ...
          {/* Logo + title — centered in scroll content */}
          <View style={{alignItems:'center', marginBottom:24}}>
            <View style={{width:72, height:72, borderRadius:20, backgroundColor:'#111', alignItems:'center', justifyContent:'center', marginBottom:10}}>
            ...
            {mode === 'signup' && isPro !== null && (
              <Text style={{fontSize:16, fontWeight:'700', color:'#6B7280', marginTop:8}}>
