...
import { ScrollView, View, Text, StyleSheet, Platform, Switch, TouchableOpacity, Pressable, Dimensions, Alert, RefreshControl, ActivityIndicator } from 'react-native';
...
export default function OrdersScreen({ navigation, route }) {
  ...
  const { accountActive, addToHistory, isOnline, setIsOnline, toggleOnline } = useAuth();
  const { t } = useLanguage();
  const scrollRef = useRef(null);
  const [selectedFilter, setSelectedFilter] = useState('smart');
  const filters = [{id:'smart',label:'Smart'},{id:'highpay',label:'€'},{id:'nearest',label:t('nearest') || 'Proche'}];
  ...
  const setOnline = toggleOnline || setIsOnline;
    ...
    try {
      ...
      if (data.data && data.data.length > 0) {
        const mapped = data.data.map((o) => ({
          id: String(o.order_id), _uid: `API-${o.order_id}`, code: `ORD-${o.order_id}`,
          merchantName: o.customer_name || 'Restaurant', restaurant: o.customer_name || 'Restaurant',
          pickupAddress: o.pickup_address || '', dropoffAddress: o.dropoff_address || '',
          address: o.dropoff_address || '',
          price: o.order_price ? `${parseFloat(o.order_price).toFixed(2)} \u20ac` : '',
          priceText: o.order_price ? `${parseFloat(o.order_price).toFixed(2)} \u20ac` : '',
          notes: o.delivery_notes || '', status: 'available',
        }));
        setAvailable(mapped);
      }
    } catch {}
  ...
  // Haptic feedback when new order arrives
  useEffect(() => {
    if (available.length > prevAvailableCount.current && prevAvailableCount.current > 0) {
  ...
  // Fetch from API first, fallback to mock data
  useEffect(() => {
    let cancelled = false;
    ...
    >
      ...
      <View style={[styles.onlineCard, !online && { borderWidth: 1, borderColor: '#E6E8EB' }]}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          <Pressable style={[styles.mapBtn, { minWidth: 44, minHeight: 44, alignItems: 'center', justifyContent: 'center' }]} onPress={() => navigation.navigate('Heatmap')} accessibilityLabel={t('heatmap') || 'Carte de chaleur'}>
            <Ionicons name="map" size={18} color="#00C29B" />
          </Pressable>
          <Switch
            value={online}
            accessibilityLabel={online ? t('goOffline') || 'Passer hors ligne' : t('goOnline') || 'Passer en ligne'}
            onValueChange={(val) => {
      ...
      {!online && (
        <Pressable style={styles.goOnlineBtn} onPress={() => setOnline(true)} accessibilityLabel={t('goOnline') || 'Passer en ligne'} accessibilityRole="button">
          ...
          <View style={styles.cardsBlock}>
            {loadingOrders ? (
              <View style={{ alignItems: 'center', paddingVertical: 40 }}>
                <ActivityIndicator size="large" color="#00C29B" />
                <Text style={[styles.emptyText, { marginTop: 12 }]}>{t('loading')}</Text>
              </View>
            ) : available.length > 0 ? (
              ...
              <View style={{ alignItems: 'center', paddingVertical: 40 }}>
                ...
                <Text style={[styles.emptyText, { marginTop: 12 }]}>{t('noOrders')}</Text>
                <Text style={{ color: '#9CA3AF', fontSize: 13, marginTop: 4 }}>{t('pullToRefresh') || 'Tirez pour rafraîchir'}</Text>
              </View>
            )}
          </View>
        </>
